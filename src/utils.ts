...
export function getClosestTemperature(
  date: Date,
  weatherData: WeatherData,
): number {
  let closestIndex = 0;
  let closestDifference = Math.abs(
    new Date(weatherData.hourly.time[0]).getTime() - date.getTime(),
  );

  for (let i = 1; i < weatherData.hourly.time.length; i++) {
    const currentDifference = Math.abs(
      new Date(weatherData.hourly.time[i]).getTime() - date.getTime(),
    );
...
export function calculateDailyAverages(
  ...
  if (dayTemps.length > 0) {
    const averageTemp = dayTemps.reduce((a, b) => a + b, 0) / dayTemps.length;
    dailyAverages.push({
      day: new Date(weatherData.hourly.time[weatherData.hourly.time.length - 1])
        .toISOString()
        .split("T")[0],
      averageTemp,
    });

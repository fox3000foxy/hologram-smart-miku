...
export default class AimlHigh {
    ...
    const traverse = (categories: any): any => {
      for (let i = 0; i < categories.length; i++) {
        if (categories[i].tagName === "aiml") {
          ...
          if (matches) {
            const isMatchingThat = this.checkForThatMatching(
              categories[i].childNodes,
            );
            if (isMatchingThat) {
              const t = this.findFinalTextInTemplateNode(
                categories[i].childNodes,
                domCategories,
          ...
          const text = this.resolveChildNodesInPatternNode(
            categories[i].childNodes,
          );
          return text;
        }
  ...
  private checkForThatMatching(categoryChildNodes: any): boolean {
    for (let i = 0; i < categoryChildNodes.length; i++) {
      if (categoryChildNodes[i].tagName === "that") {
        if (
          categoryChildNodes[i].childNodes[0].nodeValue != this.previousAnswer
        ) {
          return false;
        } else {
          return true;
    ...
    for (let i = 0; i < patternChildNodes.length; i++) {
      if (patternChildNodes[i].tagName === "bot") {
        text =
          text +
          (
            this.botAttributes[patternChildNodes[i].getAttribute("name")] || ""
          ).toUpperCase();
      } else if (patternChildNodes[i].tagName === "get") {
        text =
          text +
          (
            this.storedVariableValues[
              patternChildNodes[i].getAttribute("name")
            ] || ""
          ).toUpperCase();
  ...
  private findFinalTextInTemplateNode(
    childNodesOfTemplate: any,
    domCategories: any,
    indexOfSetTagAmountWithWildCard: number,
  ): any {
    for (let i = 0; i < childNodesOfTemplate.length; i++) {
      ...
      switch (node.tagName) {
        case "template":
          return this.findFinalTextInTemplateNode(
            node.childNodes,
            domCategories,
            indexOfSetTagAmountWithWildCard,
          );
        case "condition":
          return this.resolveSpecialNodes(
            childNodesOfTemplate,
            domCategories,
            indexOfSetTagAmountWithWildCard,
          );
        case "random":
        ...
        case "srai": {
          const sraiText =
            "" +
            this.findFinalTextInTemplateNode(
              node.childNodes,
              domCategories,
              indexOfSetTagAmountWithWildCard,
            );
  ...
  private resolveSpecialNodes(
    ...
    for (let i = 0; i < innerNodes.length; i++) {
      ...
      switch (node.tagName) {
        ...
        case "sentence":
          {
            const formalText = this.resolveSpecialNodes(
              node.childNodes,
              domCategories,
              indexOfSetTagAmountWithWildCard,
            );
        ...
        case "sr":
          {
            ...
            for (let j = 0; j < this.domArray.length; j++) {
              result = this.findCorrectCategory(
                this.lastWildCardValue,
                this.domArray[j].childNodes,
              );
              if (result) {
                text += result;
                break;
        ...
        case "random":
          {
            ...
            const randomNumber = Math.floor(
              randomSeed * node.childNodes.length,
            );
            text += this.findFinalTextInTemplateNode(
        ...
        case "condition":
          if (!node.hasAttribute("name")) {
            ...
            for (const c in node.childNodes) {
              ...
              if (child.tagName === "li") {
                if (
                  ...
                  this.storedVariableValues[child.getAttribute("name")] ===
            ...
            if (
              this.storedVariableValues[node.getAttribute("name")] ===
  ...
  private checkIfMessageMatchesPattern(
    userInput: string,
    patternText: string,
  ): boolean {

...
module.exports = (name, {app}) => {
    ...
    function calculateDailyAverages(weatherData) {
        ...
        if (dayTemps.length > 0) {
            const averageTemp = dayTemps.reduce((a, b) => a + b, 0) / dayTemps.length;
            dailyAverages.push({
                day: new Date(weatherData.hourly.time[weatherData.hourly.time.length - 1]).toISOString().split("T")[0],
                averageTemp
            });
        }
    ...
    // API endpoints
    app.post('/'+name+'/mikuAi', async (req, res) => {
        ...
        const apiKey = process.env.HERC_API_KEY; // Replace with your actual Groq API key "gsk_..."
    ...
    app.post('/'+name+'/mikuAi-offline', async (req, res) => {
        const content = req.body?.content;
        if (!content) {
            return res.json({
                success: false,
        ...
        if (content == "météo") {
            ...
            switchLights(true);
            response = {
                reply: `J'ai allumé la lumière.`
            }
        } else if (content == "éteins lumière") {
            switchLights(false);
            response = {
                reply: `J'ai éteint la lumière.`
            }
        } else {
    ...
    app.post('/'+name+'/voicevox', async (req, res) => {
        try {
    ...
    app.get('/'+name+'/isOnline', async (req, res) => {
    ...
    app.get('/'+name+'/wakeupText', (req, res) => {
        ...
        if (!wakeupId || (wakeupId > 6 && wakeupId < 1)) {
            ...
            let wakeup = wakeupResponses.find(({
                id
            }) => id == wakeupId)
            res.send({
                success: true,
    ...
    // Serve static files
    app.use('/'+name, express.static(PUBLIC_DIR));
};
...
function getMeteoCache() {
    return JSON.parse(fs.readFileSync(__dirname+"/cache/weather.json").toString());
}

// News cache
function getNewsCache() {
    return JSON.parse(fs.readFileSync(__dirname+"/cache/news.json").toString());
}
...
async function weatherDaemon() {
    ...
    fs.writeFileSync(__dirname+"/cache/weather.json", JSON.stringify(data, 0, 2));
}

//News cache
async function newsDaemon() {
    let data = await getNews(process.env.NEWS_PREFERENCES)
    fs.writeFileSync(__dirname+"/cache/news.json", JSON.stringify(data, 0, 2));
}

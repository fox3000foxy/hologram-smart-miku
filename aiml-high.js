...
var aimlHigh = function(botAttributesParam, lastAnswer){
    ...
    this.findAnswer = function(clientInput, cb){
		// console.log("Finding anwser for",clientInput)
        //check if all AIML files have been loaded. If not, call this method again after a delay
        if(isAIMLFileLoaded){
            wildCardArray = [];
            var result = '';
            for(var i = 0; i < domArray.length; i++){
				// console.log("Visiting",i,"/",domArray.length, loadedFiles[i])
            ...
            return cb(result, wildCardArray, clientInput);
        }
          ...
          return cb(undefined, wildCardArray, clientInput);
        }
    };
...
var getWildCardValue = function(userInput, patternText){
    ...
    if(replaceArray.length > 1){
		...
		// console.log(replaceArray.length)
